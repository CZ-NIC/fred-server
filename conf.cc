...
bool Conf::ReadConfigFileTXT(const char *filename )
{
FILE *f;
char buf[MAX_LINE];
...
if( strlen( dbname )) { sprintf( buf, "dbname=%s " , dbname ); strcat( conninfo , buf ) ; } 
if( strlen( user ) ){ sprintf( buf ,"user=%s " , user ); strcat( conninfo , buf ) ; } 
...
int Conf::GetLevel(char *value)
{
char levels[8][8] = {  "EMERG" , "ALERT" , "CRIT" , "ERR" , "WARNING" , "NOTICE" , "INFO" , "DEBUG" };
int i;
int level=0;

for( i =  0 ; i < 8 ; i ++ )
   {
     if( strcmp( levels[i] , value ) == 0 ) { level = i+1 ; break ; }
   }
