        ...
        const boost::optional<std::string>& _plain_password)
{
    const Database::Result db_result = _ctx.get_conn().exec_params(
            ...
            "AND ra.cert = $2::text ",
            Database::query_param_list(_registrar_handle)(_certificate_fingerprint));
    ...
    if (db_result.size() == 1)
    {
        ...
        if (_plain_password)
        {
            ...
            ::PasswordStorage::check_password(_plain_password.get(), hashed_password);
        }
        return id;
    }
    if (db_result.size() == 0)
...
{
    unsigned long long id = ::LibFred::Registrar::EppAuth::AddRegistrarEppAuth(
            _registrar_handle, _certificate_fingerprint, _plain_password).exec(_ctx);
