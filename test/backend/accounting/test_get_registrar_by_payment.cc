...
BOOST_FIXTURE_TEST_SUITE(TestGetRegistrarByPayment, SupplyFixtureCtx<HasRegistrarsAndPayments>)

BOOST_AUTO_TEST_CASE(fail_registrar_not_found)
{
    BOOST_CHECK_THROW(
        ::Fred::Backend::Accounting::Impl::get_registrar_by_payment(
            ctx,
            nonexistent_registrar_payment_data.data
        ),
