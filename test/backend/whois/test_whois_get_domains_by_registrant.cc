...
{
    Fred::Backend::Whois::DomainSeq domain_seq = impl.get_domains_by_registrant(contact_handle, regular_domains);
...
{
    Fred::Backend::Whois::DomainSeq domain_seq = impl.get_domains_by_registrant(contact_handle, regular_domains - 1);
    BOOST_CHECK(domain_seq.limit_exceeded);
...
{
    Fred::Backend::Whois::Domain dom = impl.get_domains_by_registrant(contact_handle, 1).content.at(0);
...
{
    BOOST_CHECK_THROW(impl.get_domains_by_registrant("absent-registrant", 1), Fred::Backend::Whois::ObjectNotExists);
}

BOOST_FIXTURE_TEST_CASE(get_domains_by_registrant_wrong_registrant, whois_impl_instance_fixture)
{
    BOOST_CHECK_THROW(impl.get_domains_by_registrant("", 1), Fred::Backend::Whois::InvalidHandle);
}
