...
BOOST_AUTO_TEST_SUITE(TestRestorePrevDomainState)

BOOST_FIXTURE_TEST_CASE(set_domain_not_found, SupplyFixtureCtx<HasNonexistentDomain>)
{
    BOOST_CHECK_THROW(
           blocking_impl.restorePreAdministrativeBlockStatesId(domain_list, new_owner.handle, reason, log_req_id),
...
BOOST_FIXTURE_TEST_CASE(set_one_blocked_domain, SupplyFixtureCtx<HasOneBlockedDomain>)
{
    ...
    BOOST_CHECK_EQUAL(count_blocked_domain_owners(domain_list, status_list), 0);
    blocking_impl.restorePreAdministrativeBlockStatesId(domain_list, "",  reason, log_req_id);
    BOOST_CHECK_EQUAL(count_blocked_domains(domain_list, status_list), 0);
    BOOST_CHECK_EQUAL(count_blocked_domain_owners(domain_list, status_list), 0);
    ...
    BOOST_CHECK_EQUAL(count_domains_with_user_blocking(domain_list), domain_list.size());
}

BOOST_FIXTURE_TEST_CASE(set_one_blocked_domain_with_new_owner, SupplyFixtureCtx<HasOneBlockedDomain>)
{
    blocking_impl.blockDomainsId(domain_list, status_list, owner_block_mode, block_to_date, reason, log_req_id);
    BOOST_CHECK_EQUAL(count_blocked_domains(domain_list, status_list), domain_list.size());
    BOOST_CHECK_EQUAL(count_blocked_domain_owners(domain_list, status_list), 0);
...
BOOST_FIXTURE_TEST_CASE(set_more_blocked_domains_with_new_owner, SupplyFixtureCtx<HasMoreBlockedDomains>)
{
    blocking_impl.blockDomainsId(domain_list, status_list, owner_block_mode, block_to_date, reason, log_req_id);
    BOOST_CHECK_EQUAL(count_blocked_domains(domain_list, status_list), domain_list.size());
    BOOST_CHECK_EQUAL(count_blocked_domain_owners(domain_list, status_list), owner_list.size());
...
BOOST_FIXTURE_TEST_CASE(set_more_owners_and_domains, SupplyFixtureCtx<HasMoreOwnersMoreBlockedDomains>)
{
    blocking_impl.blockDomainsId(domain_list, status_list, owner_block_mode, block_to_date, reason, log_req_id);
    BOOST_CHECK_EQUAL(count_blocked_domains(domain_list, status_list), domain_list.size());
    BOOST_CHECK_EQUAL(count_blocked_domain_owners(domain_list, status_list), owner_list.size());
    BOOST_CHECK_EQUAL(count_blocked_origin_contacts(owner_list), 0);
    blocking_impl.restorePreAdministrativeBlockStatesId(some_domains, "",  reason, log_req_id);
    BOOST_CHECK_EQUAL(count_blocked_domains(domain_list, status_list), domain_list.size() - some_domains.size());
    BOOST_CHECK_EQUAL(count_blocked_domain_owners(some_domains, status_list), 0);
    BOOST_CHECK_EQUAL(count_blocked_origin_contacts(owner_list), 0);
    BOOST_CHECK_EQUAL(count_domains_with_admin_contacts(domain_list), domain_list.size());
    BOOST_CHECK_EQUAL(count_domains_with_user_blocking(domain_list), some_domains.size());
}

BOOST_FIXTURE_TEST_CASE(set_more_owners_and_domains_with_new_owner, SupplyFixtureCtx<HasMoreOwnersMoreBlockedDomains>)
{
    blocking_impl.blockDomainsId(domain_list, status_list, owner_block_mode, block_to_date, reason, log_req_id);
    BOOST_CHECK_EQUAL(count_blocked_domains(domain_list, status_list), domain_list.size());
    BOOST_CHECK_EQUAL(count_blocked_domain_owners(domain_list, status_list), owner_list.size());
    BOOST_CHECK_EQUAL(count_blocked_origin_contacts(owner_list), 0);
    blocking_impl.restorePreAdministrativeBlockStatesId(some_domains, new_owner.handle,  reason, log_req_id);
    BOOST_CHECK_EQUAL(count_blocked_domains(domain_list, status_list), domain_list.size() - some_domains.size());
    BOOST_CHECK_EQUAL(count_blocked_domain_owners(domain_list, status_list), owner_list.size());
    BOOST_CHECK_EQUAL(count_blocked_origin_contacts(owner_list), 0);
    BOOST_CHECK_EQUAL(count_domains_with_admin_contacts(domain_list), domain_list.size());
