...
namespace Queue {
    ...
    static std::string get_already_checked_contacts_query(LibFred::OperationContext& _ctx, const std::string& _testsuite_handle) {
        return
            "SELECT "
    ...
    std::vector<enqueued_check> fill_check_queue::exec() {
        ...
        if(checks_to_enqueue_count > 0) {
            ...
            BOOST_FOREACH(unsigned long long contact_id, to_enqueue_never_checked) {
                ...
                LibFred::InfoContactCheckOutput info = LibFred::InfoContactCheck(uuid::from_string(temp_handle)).exec(ctx1);
        ...
        if(checks_to_enqueue_count > 0) {
            ...
            if( !to_enqueue_oldest_checked.empty() ) {
                ...
                for(std::vector<unsigned long long>::const_iterator contact_id_it = to_enqueue_oldest_checked.begin();
                    ...
                    LibFred::InfoContactCheckOutput info = LibFred::InfoContactCheck(uuid::from_string(temp_handle)).exec(ctx2);
