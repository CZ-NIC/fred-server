...
namespace Verification {
    ...
    const LibFred::Document::GenerationType TestContactability::letter_doc_type_(LibFred::Document::GT_ADMIN_CONTACT_VERIFICATION_CONTACT_CHECK_NOTICE);
    ...
    Test::TestRunResult TestContactability::run(unsigned long long _history_id) const {
        ...
        if(error) {
            return TestRunResult(LibFred::ContactTestStatus::ERROR, error_msg, mail_ids, message_ids );
        } else {
            return TestRunResult(LibFred::ContactTestStatus::MANUAL, std::string(), mail_ids, message_ids );
        }
    }
