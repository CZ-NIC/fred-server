...
namespace ContactVerification {
    ...
    Test::TestRunResult TestEmailExistsForManagedZones::run(unsigned long long _history_id) const {
        ...
        if(skipped_email_count == emails.size() && emails.size() > 0) {
            return TestRunResult (LibFred::ContactTestStatus::SKIPPED, std::string("this test is intended for e-mails from managed zones only") );
        } else if( !invalid_emails.empty() ) {
            return TestRunResult (LibFred::ContactTestStatus::FAIL, "emails: " + boost::join(invalid_emails, ",") + " are invalid");
        }
