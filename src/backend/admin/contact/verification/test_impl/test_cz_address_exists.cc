...
namespace ContactVerification {
    ...
    Test::TestRunResult TestCzAddress::run(unsigned long long _history_id) const {
        ...
        if(country != "cz") {
            return TestRunResult (LibFred::ContactTestStatus::SKIPPED, string("this test is intended for CZ addresses only") );
        }
        ...
        try {
            ...
            if(city.size() < 1) {
                return TestRunResult (LibFred::ContactTestStatus::FAIL, string("city is missing content") );
            }
            postal_code = parse_postal_code(static_cast<string>(data.postalcode_));

        } catch (const ExceptionMissingHouseNumber& e) {
            return TestRunResult (LibFred::ContactTestStatus::FAIL, e.what() );

        } catch (const ExceptionInvalidPostalCode& e) {
            return TestRunResult (LibFred::ContactTestStatus::FAIL, e.what() );

        } catch (...) {
            return TestRunResult (LibFred::ContactTestStatus::FAIL, string("exception during parsing") );
        }

        try {
            if(is_address_valid(street, city, postal_code)) {
                return TestRunResult (LibFred::ContactTestStatus::OK, string() );
            }
        } catch (...) {
            return TestRunResult (LibFred::ContactTestStatus::FAIL, string("exception during validation") );
        }
        ...
        try {
            ...
            return TestRunResult (LibFred::ContactTestStatus::FAIL, string("exception during diagnostics") );
        }



        return TestRunResult (LibFred::ContactTestStatus::FAIL, error_msg );
    }
