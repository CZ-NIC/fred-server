...
namespace Verification {
    ...
    static Optional<std::string> lock_some_test(LibFred::OperationContext& ctx, const uuid& check_handle, const std::string& test_status) {
    ...
    static Optional<std::string> lock_some_running_test(LibFred::OperationContext& ctx, const uuid& check_handle) {
        return lock_some_test(ctx, check_handle, LibFred::ContactTestStatus::RUNNING);
    }

    static Optional<std::string> lock_some_enqueued_test(LibFred::OperationContext& ctx, const uuid& check_handle) {
        return lock_some_test(ctx, check_handle, LibFred::ContactTestStatus::ENQUEUED);
    }

    static void create_all_tests (LibFred::OperationContext& ctx, const uuid& check_handle, Optional<unsigned long long> logd_request_id) {
    ...
    static std::string evaluate_check_status_after_tests_finished(LibFred::OperationContext& ctx, const uuid& handle) {
        ...
        for(std::vector<LibFred::InfoContactCheckOutput::ContactTestResultData>::const_iterator it = info_check.tests.begin();
    ...
    static void preprocess_manual_check(
        ...
        LibFred::InfoContactCheckOutput check_info = LibFred::InfoContactCheck(
            _check_handle
        ).exec(_ctx);
    ...
    static void preprocess_check(LibFred::OperationContext& ctx, const uuid& handle) {
    ...
    static void run_test(
        ...
        if( temp_result.status == LibFred::ContactTestStatus::ENQUEUED
            ...
            throw LibFred::InternalError("malfunction in implementation of test " + _test_handle + ", run() returned bad status");
        }
    }
        ...
        while(true) {
            while(true) {
                ...
                while(true) {
                    while(true) {
                        ...
                        if (error_occurred) {
                            try {
                                ...
                                throw LibFred::InternalError( (string("problem when setting test status to ERROR; check_handle=") + check_handle_to_process.get_value() + "; test_handle=" + test_handle_to_process.get_value()).c_str() );
                            }
                        }
                    }
                    ...
                    LibFred::UpdateContactTest( check_uuid_to_process, test_handle_to_preprocess.get_value(), LibFred::ContactTestStatus::RUNNING, _logd_request_id, Optional<std::string>() ).exec(ctx_test_preprocess);
            ...
            LibFred::UpdateContactCheck( check_uuid_to_preprocess, LibFred::ContactCheckStatus::RUNNING, _logd_request_id).exec(ctx_check_preprocess);
