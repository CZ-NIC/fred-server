...
namespace Registry
{
    namespace DomainBrowserImpl
    {
         ...
         */
        template <class EXCEPTION> LibFred::InfoContactOutput check_contact_id(LibFred::OperationContext& ctx,
         ...
         */
        template <class EXCEPTION> LibFred::InfoContactOutput check_user_contact_id(LibFred::OperationContext& ctx,
                unsigned long long user_contact_id, const std::string& output_timezone, bool lock_contact_for_update = false)
        {
            LibFred::InfoContactOutput info = check_contact_id<EXCEPTION>(ctx, user_contact_id, output_timezone, lock_contact_for_update);

            if(!LibFred::ObjectHasState(user_contact_id, LibFred::ObjectState::MOJEID_CONTACT).exec(ctx))
            {
                throw EXCEPTION();
            }
        ...
        std::vector<std::string> DomainBrowser::get_object_states(LibFred::OperationContext& ctx, unsigned long long object_id)
        {
        ...
        unsigned long long DomainBrowser::getContactId(const std::string& handle)
        {
            ...
            try
            {
                return LibFred::get_object_id_by_handle_and_type_with_lock(ctx, true,handle, "contact",
            ...
            const std::string& registrar_handle)
        {
            ...
            try
            {
                ...
                try
                {
                    registar_info = LibFred::InfoRegistrarByHandle(registrar_handle).exec(ctx, output_timezone);
                }
                ...
                unsigned long long contact_id)
        {
            ...
            try
            {
                ...
                try
                {
                    contact_info = LibFred::InfoContactById(contact_id).exec(ctx, output_timezone);
                }
                catch(const LibFred::InfoContactById::Exception& ex)
                {
                    if(ex.is_set_unknown_object_id())
                ...
                detail.mailing_address = optional_map_at<Nullable>(contact_info.info_contact_data.addresses, LibFred::ContactAddressType::MAILING);
                ...
                unsigned long long domain_id)
        {
            ...
            try
            {
                ...
                try
                {
                    domain_info = LibFred::InfoDomainById(domain_id).exec(ctx, output_timezone);
                }
                ...
                for(std::vector<LibFred::ObjectIdHandlePair>::const_iterator ci = domain_info.info_domain_data.admin_contacts.begin();
                        ci != domain_info.info_domain_data.admin_contacts.end(); ++ci)
                {
                    LibFred::InfoContactOutput admin_contact_info = LibFred::InfoContactById(ci->id).exec(ctx, output_timezone);
                ...
                unsigned long long nsset_id)
        {
            ...
            try
            {
                ...
                try
                {
                    nsset_info = LibFred::InfoNssetById(nsset_id).exec(ctx, output_timezone);
                }
                ...
                for(std::vector<LibFred::ObjectIdHandlePair>::const_iterator ci = nsset_info.info_nsset_data.tech_contacts.begin();
                        ci != nsset_info.info_nsset_data.tech_contacts.end(); ++ci)
                {
                    LibFred::InfoContactOutput tech_contact_info = LibFred::InfoContactById(ci->id).exec(ctx, output_timezone);
                ...
                unsigned long long keyset_id)
        {
            ...
            try
            {
                ...
                try
                {
                    keyset_info = LibFred::InfoKeysetById(keyset_id).exec(ctx, output_timezone);
                }
                ...
                for(std::vector<LibFred::ObjectIdHandlePair>::const_iterator ci = keyset_info.info_keyset_data.tech_contacts.begin();
            ...
            unsigned long long request_id)
        {
            ...
            try
            {
                LibFred::InfoContactOutput contact_info = check_user_contact_id<UserNotExists>(ctx, user_contact_id, output_timezone, true);

                if(!(LibFred::ObjectHasState(user_contact_id, LibFred::ObjectState::IDENTIFIED_CONTACT).exec(ctx)
                    || LibFred::ObjectHasState(user_contact_id, LibFred::ObjectState::VALIDATED_CONTACT).exec(ctx)))
                {
                    throw AccessDenied();
                }

                if(LibFred::ObjectHasState(user_contact_id, LibFred::ObjectState::SERVER_BLOCKED).exec(ctx))
                {
                    throw ObjectBlocked();
                }
            ...
            unsigned long long request_id)
        {
            ...
            try
            {
                ...
                if(!(LibFred::ObjectHasState(contact_id, LibFred::ObjectState::IDENTIFIED_CONTACT).exec(ctx)
                    || LibFred::ObjectHasState(contact_id, LibFred::ObjectState::VALIDATED_CONTACT).exec(ctx)))
                {
                    throw AccessDenied();
                }

                if(LibFred::ObjectHasState(contact_id, LibFred::ObjectState::SERVER_BLOCKED).exec(ctx))
                {
                    throw ObjectBlocked();
                }
                ...
                LibFred::UpdateContactById(contact_id, update_registrar_).set_authinfo(authinfo).set_logd_request_id(request_id).exec(ctx);
                ctx.commit_transaction();
            }
            catch(...)
            {
            ...
            std::vector<std::string>& blocked_objects)
        {
            ...
            try
            {
                LibFred::InfoContactOutput contact_info = check_user_contact_id<UserNotExists>(ctx, user_contact_id, output_timezone);

                if(!LibFred::ObjectHasState(user_contact_id, LibFred::ObjectState::VALIDATED_CONTACT).exec(ctx))
                {
                    throw AccessDenied();
                }
                ...
                {
                    object_type_id = LibFred::get_object_type_id(ctx,objtype);
                }
                    ...
                    ; ci != object_id_name_pairs.end(); ++ci)
                {
                    ...
                    if(LibFred::ObjectHasState(ci->first, LibFred::ObjectState::SERVER_BLOCKED).exec(ctx_per_object))//object administratively blocked
                    {
                    ...
                    if((block_type == UNBLOCK_TRANSFER)
                        && LibFred::ObjectHasState(ci->first, LibFred::ObjectState::SERVER_UPDATE_PROHIBITED).exec(ctx_per_object))//forbidden partial unblocking
                    {
                        blocked_objects.push_back(ci->second);
                        continue;
                    }
                    ...
                    {
                        if(!LibFred::ObjectHasState(ci->first, LibFred::ObjectState::SERVER_TRANSFER_PROHIBITED).exec(ctx_per_object))
                        {
                            LibFred::CreateObjectStateRequestId(ci->first,
                                Util::set_of<std::string>(LibFred::ObjectState::SERVER_TRANSFER_PROHIBITED)).exec(ctx_per_object);
                        }
                    }

                    if(block_type == BLOCK_TRANSFER_AND_UPDATE)//block update
                    {
                        if(!LibFred::ObjectHasState(ci->first, LibFred::ObjectState::SERVER_UPDATE_PROHIBITED).exec(ctx_per_object))
                        {
                            LibFred::CreateObjectStateRequestId(ci->first,
                                Util::set_of<std::string>(LibFred::ObjectState::SERVER_UPDATE_PROHIBITED)).exec(ctx_per_object);
                        }
                    }
                    ...
                    if((block_type == UNBLOCK_TRANSFER) || (block_type == UNBLOCK_TRANSFER_AND_UPDATE))//unblock transfer
                    {
                        if(LibFred::ObjectHasState(ci->first, LibFred::ObjectState::SERVER_TRANSFER_PROHIBITED).exec(ctx_per_object))
                        {
                            LibFred::CancelObjectStateRequestId(ci->first,
                                Util::set_of<std::string>(LibFred::ObjectState::SERVER_TRANSFER_PROHIBITED)).exec(ctx_per_object);
                        }
                    }
...
"ORDER BY keyset_list.id",
        ...
        {
            bool operator()(LibFred::OperationContext& ctx,
                ...
                const std::string& dst_contact_handle) const
            {
                ...
                {
                    BOOST_THROW_EXCEPTION(LibFred::MergeContact::Exception().set_identical_contacts_handle(dst_contact_handle));
                }
                ...
                {
                    BOOST_THROW_EXCEPTION(LibFred::MergeContact::Exception().set_unable_to_get_difference_of_contacts(
                        LibFred::MergeContact::InvalidContacts(src_contact_handle,dst_contact_handle)));
                }

                unsigned long long dst_contact_id = static_cast<unsigned long long>(diff_result[0]["dst_contact_id"]);

                if(LibFred::ObjectHasState(dst_contact_id, LibFred::ObjectState::SERVER_BLOCKED).exec(ctx))
                {
                    BOOST_THROW_EXCEPTION(LibFred::MergeContact::Exception().set_dst_contact_invalid(src_contact_handle));
                }

                unsigned long long src_contact_id = static_cast<unsigned long long>(diff_result[0]["src_contact_id"]);

                if(LibFred::ObjectHasState(src_contact_id, LibFred::ObjectState::MOJEID_CONTACT).exec(ctx)
                    || LibFred::ObjectHasState(src_contact_id, LibFred::ObjectState::SERVER_BLOCKED).exec(ctx)
                    || LibFred::ObjectHasState(src_contact_id, LibFred::ObjectState::SERVER_DELETE_PROHIBITED).exec(ctx))
                {
                    BOOST_THROW_EXCEPTION(LibFred::MergeContact::Exception().set_src_contact_invalid(src_contact_handle));
                }
            ...
            unsigned long long request_id)
        {
            ...
            try
            {
                ...
                if(!send_expiration_letters && !LibFred::ObjectHasState(contact_id, LibFred::ObjectState::VALIDATED_CONTACT).exec(ctx))
                {
                    throw AccessDenied();
                }
                ...
                LibFred::UpdateContactById(contact_id, update_registrar_)
