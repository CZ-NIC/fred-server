        ...
        const SessionData& _session_data)
{
    ...
    Logging::Context logging_ctx4(boost::str(boost::format("action-%1%") % static_cast<unsigned>(Action::ContactSendAuthInfo)));
    ...
    {
        LibFred::OperationContextCreator ctx;
        ctx.get_log().info(std::string("authinfo_contact_localized: ") + e.what());
    ...
    {
        LibFred::OperationContextCreator ctx;
        ctx.get_log().info(std::string("authinfo_contact_localized failure: ") + e.what());
        throw EppResponseFailureLocalized(
                ctx,
    ...
    {
        LibFred::OperationContextCreator ctx;
        ctx.get_log().info("unexpected exception in authinfo_contact_localized function");
        throw EppResponseFailureLocalized(
                ctx,
                EppResponseFailure(EppResultFailure(EppResultCode::command_failed)),
                _session_data.lang);
