                ...
                "SELECT EXISTS(SELECT 1 FROM public_request "
                              "WHERE id=$1::BIGINT AND "
                                    "create_time<(SELECT GREATEST(update,trdate) FROM object "
                                                 "WHERE id=$2::BIGINT)"
                             ") AS object_changed",
                Database::query_param_list(static_cast< const LibFred::LockedPublicRequest& >
...
Fred::Backend::Buffer MojeIDImpl::get_validation_pdf(ContactId _contact_id)const
{
    ...
    try {
        ...
        const bool is_ssn_ico =      ssn_type == LibFred::SSNType::ico;
        const std::string birthday = ssn_type == LibFred::SSNType::birthday
        ...
        LogRequestId _log_request_id)const
{
    ...
    try {
        ...
        LibFred::UpdatePublicRequest().set_status(LibFred::PublicRequest::Status::invalidated)
    ...
    const Optional< bool > &_validated_contact)
{
    ...
    if (!_validated_contact.isset()) {
        params(_data.id)
              (Conversion::Enums::to_db_handle(LibFred::Object_State::validated_contact));
