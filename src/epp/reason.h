...
inline unsigned to_description_db_id(const Reason::Enum state) {
 ...
 */
template<> inline Reason::Enum from_description_db_id<Reason>(const unsigned id) {

    if( id < 1 || id > 62 ) {
        throw UnknownLocalizedDescriptionId();
    }

    return static_cast<Reason::Enum>(id);
}
