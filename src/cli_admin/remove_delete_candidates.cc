...
{
    Database::query_param_list params(Conversion::Enums::to_db_handle(Fred::Object_State::delete_candidate));
    ...
    for (const auto relative_time_of_deletion : relative_times_of_deletions)
    {
        ...
        try
        {
            ...
            if (idx_of_deleted_object < domains.size())
            {
                ...
                Fred::DeleteDomainById(domains[idx_of_deleted_domain].domain_id.get_value()).exec(ctx);
            }
            else if (idx_of_deleted_object < (domains.size() + nssets.size()))
            {
                ...
                Fred::DeleteNssetById(nssets[idx_of_deleted_nsset].nsset_id.get_value()).exec(ctx);
            }
            else if (idx_of_deleted_object < (domains.size() + nssets.size() + keysets.size()))
            {
                ...
                Fred::DeleteKeysetById(keysets[idx_of_deleted_keyset].keyset_id.get_value()).exec(ctx);
            }
            else
            {
                const unsigned idx_of_deleted_contact = idx_of_deleted_object - (domains.size() + nssets.size() + keysets.size());
                Fred::DeleteContactById(contacts[idx_of_deleted_contact].contact_id.get_value()).exec(ctx);
            }
            ctx.commit_transaction();
        }
        catch (const Fred::DeleteContactById::Exception& e)
        {
            LOGGER(PACKAGE).error(boost::format("delete contact failed (%1%)") % e.what());
        }
        catch (const Fred::DeleteDomainById::Exception& e)
        {
            LOGGER(PACKAGE).error(boost::format("delete domain failed (%1%)") % e.what());
        }
        catch (const Fred::DeleteKeysetById::Exception& e)
        {
            LOGGER(PACKAGE).error(boost::format("delete keyset failed (%1%)") % e.what());
        }
        catch (const Fred::DeleteNssetById::Exception& e)
        {
