...
namespace Zone {
   ...
   class ZoneNsImpl
	...
	public:
			ZoneListImpl() :
			  ...
			  virtual void reload()
			  {
			      ...
			      {
			       LOGGER.error("reload: an error has occured");
			       throw SQL_ERROR();
			      }//catch (...)
			  }//reload()
		      ...
		      virtual void reload(Database::Filters::Union &uf)
		      {
		          ...
		          LOGGER.debug(boost::format("reload(uf) ZoneList query: %1%") % info_query_str);
		          try {
		            Database::Connection conn = Database::Manager::acquire();
    ...
    class ManagerImpl : virtual public Manager
    {
     ...
     public:
      ...
      virtual bool checkTLD(const DomainName& domain) const
      {
          ...
          {
              LOGGER.error("checkTLD: an error has occured");
              return false;
          }//catch (...)
      }//checkTLD
              ...
              const std::string& _ns_fqdn) final override
      {
        ...
        {
            LOGGER.error("addZone: an error has occured");
            throw SQL_ERROR();
        }
      }
            ...
            bool postpaid_operation_enabled)
    {
        ...
        {
            LOGGER.error(std::string("addPrice: ") + e.what());
            throw;
        }
        ...
        {
            LOGGER.error("addPrice: an error has occurred");
            throw;
        }
              ...
              , const bool enable_postpaid_operation)
      {
          ...
          {
              LOGGER.error("addPrice: an error has occured");
              throw;
          }//catch (...)
      }//addPrice
