...
public:
    ...
    virtual void setRegistrarId(TID registrar_id) = 0;
    virtual void set_password(const std::string& plaintext_password) = 0;
protected:
...
struct RegistrarZone
{
    ...
    RegistrarZone(
            ...
            const Money& _credit,
            const Database::Date& _fromdate,
            const Database::Date& _todate)
        : id(),
          name(_name),
          credit(_credit),
          fromdate(_fromdate),
...
class Registrar : virtual public LibFred::CommonObjectNew
{
public:
    ...
    virtual TID getId() const = 0;
    virtual void setId(unsigned long long _id) = 0;
    ...
    virtual bool isInZone(const std::string& zone_fqdn) const = 0;
...
public:
    ...
    /// sort by column
    virtual void sort(MemberType _member, bool _asc, unsigned _zone_id = 0, RegistrarZoneAccess* rzaptr = nullptr) = 0;
...
public:
    ...
    virtual void cancelRegistrarGroup(TID _group_id) = 0;
    ///update registrar group
    virtual void updateRegistrarGroup(TID _group_id, const std::string& _group_name) = 0;
    ///create registrar certification
    virtual unsigned long long createRegistrarCertification(
            TID _registrar_id,
    ...
    virtual void shortenRegistrarCertification(TID _certification_id, const Database::Date& _valid_until) = 0;
    ///update registrar certification
    virtual void updateRegistrarCertification(TID _certification_id, RegCertClass _classification, TID _eval_file_id) = 0;
    ///get registrar certifications
    virtual CertificationSeq getRegistrarCertifications(TID _registrar_id) = 0;

    ///create membership of registrar in group
    virtual unsigned long long createRegistrarGroupMembership(
            TID _registrar_id,
    ...
    virtual void endRegistrarGroupMembership(TID _registrar_id, TID _registrar_group_id) = 0;
    ...
    virtual bool blockRegistrar(TID registrar_id, const EppCorbaClient* epp_cli) = 0;
    virtual void unblockRegistrar(TID registrar_id, TID request_id) = 0;

    virtual void blockRegistrarsOverLimit(
            const EppCorbaClient* epp_client,
            Logger::LoggerClient* logger_client) = 0;

    virtual BlockedRegistrars getRegistrarsBlockedToday() = 0;

    virtual bool hasRegistrarZoneAccess(unsigned long long _registrar_id, unsigned long long _zone_id) = 0;

    /// logs error and throws when registrar doesn't exist
    virtual void checkRegistrarExists(TID registrar_id) = 0;
    ...
    virtual std::unique_ptr<RequestFeeDataMap> getRequestFeeDataMap(
            ...
            const boost::gregorian::date& zone_access_date) = 0;
...
#endif//REGISTRAR_HH_33FA604169D84E8FBEFD15326F3F066D
