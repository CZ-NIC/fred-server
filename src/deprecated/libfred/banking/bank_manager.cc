...
unsigned long long get_registrar_id_by_handle(const std::string& _registrar_handle)
{
        ...
        const PaymentData& _payment)
{
    ...
    query.buffer() <<
            ...
            "SELECT id "
            "FROM registrar "
            "WHERE varsymb=" << Database::Value(_payment.variable_symbol) << " OR "
                  "(0<length(trim(regex)) AND " << Database::Value(_payment.memo) << " ~* trim(regex))";
            // clang-format on
    Database::Connection conn = Database::Manager::acquire();
    const Database::Result result = conn.exec(query);
    if (result.size() != 1)
    {
        LOGGER.warning(boost::format(
                    "couldn't find suitable registrar for payment id=%1% => processing canceled") % _payment.uuid);
        throw RegistrarNotFound();
    }
        ...
        const boost::optional<std::string>& _account_bank_code)
{
    ...
    query.buffer() <<
            ...
            "WHERE TRIM(LEADING '0' FROM account_number) = "
                  "TRIM(LEADING '0' FROM " << Database::Value(_account_number) << ") AND "
                  "bank_code=" << Database::Value(_account_bank_code.get_value_or(""));
            // clang-format on
    Database::Connection conn = Database::Manager::acquire();
    const Database::Result result = conn.exec(query);
    if (result.size() == 0)
...
std::string get_invoice_number_by_id(unsigned long long _invoice_id)
{
    Database::Query query;
    query.buffer() <<
            // clang-format off
...
public:
            ...
            const boost::optional<std::string>& _registrar_handle)
    {
        ...
        try {
            ...
            LOGGER.debug("saving transaction for a single payment");
...
private:
            ...
            unsigned long long _registrar_id)
    {
        ...
        try
        {
            ...
            {
                LOGGER.warning("could not process payment, price not > 0");
            ...
            std::vector<LibFred::Invoicing::unpaid_account_invoice> uai_vect =
                    invoice_manager->find_unpaid_account_invoices(_registrar_id, zone_id);

            Money payment_price_rest = _payment.price;

            //for unpaid_account_invoices
            for (const auto& uai : uai_vect)
            {
                const unsigned long long unpaid_account_invoice_id = uai.id;
                const Money uaci_balance = uai.balance;//w/o vat
                const Decimal uaci_vat = uai.vat;
                const Money unpaid_price_with_vat = uaci_balance + uaci_balance * uaci_vat / Decimal("100");

                //if payment is not big enough to pay this account invoice
                const bool is_partial_payment = (payment_price_rest <= unpaid_price_with_vat);

                const Money partial_price = is_partial_payment ? payment_price_rest
                                                               : unpaid_price_with_vat;
                if (partial_price > Money("0"))//don't do zero transactions
                {
                    ...
                    if (is_partial_payment)
                    {
                        ...
                        pay_invoice(_registrar_id, zone_id, _payment.uuid, balance_change, unpaid_account_invoice_id);
                    ...
                    else
                    {
                        Money balance_change //is rest of invoice balance without vat
                            = invoice_manager->zero_account_invoice_balance(unpaid_account_invoice_id);
                        pay_invoice(_registrar_id, zone_id, _payment.uuid, balance_change, unpaid_account_invoice_id);
                        payment_invoices.account_invoices.push_back(
                                InvoiceReference(
                                        unpaid_account_invoice_id,
            ...
            if (payment_price_rest > Money("0"))
            {
                ...
                const unsigned long long advance_invoice_id = invoice_manager->createDepositInvoice(
                        tax_date,
                        zone_id,
                        _registrar_id,
                        payment_price_rest,
                ...
                pay_invoice(_registrar_id , zone_id, _payment.uuid, out_credit, advance_invoice_id);
