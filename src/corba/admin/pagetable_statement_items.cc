    ...
    throw (ccReg::Table::INVALID_ROW)
{
    ...
    MAKE_OID(oid_invoice_id, item->getInvoiceId(), C_STR(item->getInvoicePrefix()), FT_INVOICE);

 //   (*tr)[0] <<= oid_id;
 //   (*tr)[1] <<= oid_statement_id;
    (*tr)[0] <<= C_STR(item->getAccountNumber());
    (*tr)[1] <<= C_STR(item->getBankCodeId());
//    (*tr)[4] <<= C_STR(item->getCode());
//    (*tr)[5] <<= C_STR(item->getType());
//    (*tr)[6] <<= C_STR(item->getKonstSym());
    (*tr)[2] <<= C_STR(item->getVarSymb());
//    (*tr)[8] <<= C_STR(item->getSpecSymb());
    (*tr)[3] <<= formatMoney(item->getPrice()).c_str();
//    (*tr)[10] <<= C_STR(item->getAccountEvid());
    (*tr)[4] <<= C_STR(item->getAccountDate());
    (*tr)[5] <<= C_STR(item->getAccountMemo());
    (*tr)[6] <<= oid_invoice_id;
    (*tr)[7] <<= C_STR(item->getAccountName());
    (*tr)[8] <<= C_STR(item->getCrTime());
