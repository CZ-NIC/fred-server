...
#include "src/libfred/blacklist.hh"
#include "src/libfred/object_impl.hh"
#include "src/libfred/sql.hh"
...
public:
  ...
  bool isDeletePending(const std::string &_fqdn) const
  {
      ...
      if (result.size() > 0) {
          LOGGER(PACKAGE).debug(boost::format("delete pending check for fqdn %1% selected id=%2%")
          ...
          if (check.size() > 0) {
              LOGGER(PACKAGE).debug(boost::format("delete pending check found newer domain %1% with id=%2%")
