...
namespace Convert {
    ...
    std::string to_string(const LibFred::ContactAddress& _address) {
        ...
        aggregate_nonempty(
            non_empty_parts,
            to_string(
                static_cast<const LibFred::Contact::PlaceAddress&>(_address)
            )
        );
...
static std::string translate_ssntypes(const Nullable< LibFred::PersonalIdUnion > &_nullable_personal_id)
{
...
static std::map<std::string, std::string> gather_contact_create_data_change(
    const LibFred::InfoContactData& _fresh
) {
    ...
    BOOST_FOREACH( LibFred::ContactAddressType::Value type, LibFred::ContactAddressType::get_all() ) {
        const std::map<LibFred::ContactAddressType, LibFred::ContactAddress>::const_iterator fresh_it = fresh_addresses.find(type);
...
static std::map<std::string, std::string> gather_contact_update_data_change(
    ...
    if(diff.addresses.isset()) {
        const std::map<LibFred::ContactAddressType, LibFred::ContactAddress> old_addresses = diff.addresses.get_value().first;
        const std::map<LibFred::ContactAddressType, LibFred::ContactAddress> new_addresses = diff.addresses.get_value().second;

        BOOST_FOREACH( LibFred::ContactAddressType::Value type, LibFred::ContactAddressType::get_all() ) {
            const std::map<LibFred::ContactAddressType, LibFred::ContactAddress>::const_iterator old_it = old_addresses.find(type);
            const std::map<LibFred::ContactAddressType, LibFred::ContactAddress>::const_iterator new_it = new_addresses.find(type);
    ...
    if (diff.personal_id.isset()) {
        const Nullable< LibFred::PersonalIdUnion > nullable_personal_id_a = diff.personal_id.get_value().first;
        const Nullable< LibFred::PersonalIdUnion > nullable_personal_id_b = diff.personal_id.get_value().second;
        add_old_new_changes_pair_if_different(
...
std::set<std::string> get_emails_to_notify_contact_event(
    ...
    {
        Nullable<std::string> notify_email = LibFred::InfoContactHistoryByHistoryid(_history_id_after_change).exec(_ctx).info_contact_data.notifyemail;
