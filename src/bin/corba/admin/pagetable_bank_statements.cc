#include "src/bin/corba/admin/pagetable_bank_statements.hh"

ccReg_Statements_i::ccReg_Statements_i(
        LibFred::Banking::StatementList *list):
    list_(list)
{
}
...
ccReg_Statements_i::getRow(CORBA::UShort row)
{
    ...
    const LibFred::Banking::Statement *data = list_->get(row);
    if (!data) {
...
ccReg_Statements_i::sortByColumn(CORBA::Short column, CORBA::Boolean dir)
{
    ...
    switch (column) {
        case 0:
            list_->sort(LibFred::Banking::MT_ACCOUNT_ID, dir);
            break;
        case 1:
            list_->sort(LibFred::Banking::MT_NUM, dir);
            break;
        case 2:
            list_->sort(LibFred::Banking::MT_CREATE_DATE, dir);
            break;
        case 3:
            list_->sort(LibFred::Banking::MT_BALANCE_OLD_DATE, dir);
            break;
        case 4:
            list_->sort(LibFred::Banking::MT_BALANCE_OLD, dir);
            break;
        case 5:
            list_->sort(LibFred::Banking::MT_BALANCE_NEW, dir);
            break;
        case 6:
            list_->sort(LibFred::Banking::MT_BALANCE_CREDIT, dir);
            break;
        case 7:
            list_->sort(LibFred::Banking::MT_BALANCE_DEBET, dir);
            break;
        case 8:
            list_->sort(LibFred::Banking::MT_FILE_ID, dir);
            break;
...
ccReg_Statements_i::saveFilter(const char *name)
{
    ...
    tmp_filter_manager->save(LibFred::Filter::FT_STATEMENTHEAD, name, uf);
}

LibFred::Banking::Statement *
ccReg_Statements_i::findId(ccReg::TID id)
{
    Logging::Context ctx(base_context_);
    try {
        LibFred::Banking::Statement *item = list_->getById(id);
        if (item) {
            return item;
        }
        return 0;
