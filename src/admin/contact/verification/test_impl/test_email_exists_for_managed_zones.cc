...
namespace ContactVerification {
        ...
        */
        for(std::vector<std::string::iterator>::const_iterator it = label_separator_positions.begin() + 1;
            ...
            if( managed_zones.find(
                    std::string( (*it) + 1, at_domain.end() ) // emulation of "hostname.substr(it)"
                ) == managed_zones.end()
            ) {
                continue;
            }
            ...
            if(
                ctx.get_conn().exec_params(
                    ...
                    Database::query_param_list
                        // (it - 1)         is vector iterator pointing to position of previous delimiter (position is represented by different string iterator)
        ...
        return Fred::ContactTestStatus::SKIPPED;
    }

    static std::set<std::string> get_managed_zones(
        Fred::OperationContext& ctx
    ) {
    ...
    Test::TestRunResult TestEmailExistsForManagedZones::run(unsigned long long _history_id) const {
        ...
        for(std::vector<std::string>::const_iterator it = emails.begin();
            ...
            std::string result = check_email(ctx, *it, managed_zones);

            if(result == Fred::ContactTestStatus::SKIPPED) {
                skipped_email_count++;
            } else if (result == Fred::ContactTestStatus::FAIL) {
                invalid_emails.push_back(*it);
            }
        }

        if(skipped_email_count == emails.size() && emails.size() > 0) {
            return TestRunResult (Fred::ContactTestStatus::SKIPPED, std::string("this test is intended for e-mails from managed zones only") );
        } else if( !invalid_emails.empty() ) {
            return TestRunResult (Fred::ContactTestStatus::FAIL, "emails: " + boost::join(invalid_emails, ",") + " are invalid");
        }
