...
public:
  ...
  // TODO this should really be in Banking module
        ...
        Database::ID invoiceId)
{
  ...
  /// Methods added in order to comply with interface introduced in invoice_manager.cc:
  // TODO this should be in Banking module
  bool pairInvoices() {
      return pairInvoices(false);
  }

  bool pairInvoices(bool report) {
...
//        << " JOIN bank_head bh ON bi.statement_id=bh.id"
    ...
    for (; it != res.end(); ++it) {
        ...
        int invoiceId = createDepositInvoice(date, (int)zoneId, (int)registrarId, (long)price);
        if (invoiceId == 0) {
...
int createDepositInvoice(Database::Date date, int zoneId, int registrarId, long price) {
  ...
  // creates its own DB connection, so it shouldn't be called inside of a transaction
...
//        << " JOIN bank_head bh ON bi.statement_id=bh.id"
    ...
    autoDB db;
    if(!db.OpenDatabase(Database::Manager::getConnectionString())) {
        LOGGER(PACKAGE).error(" DB: Failed to open the database. ");
    }

    // if the call failed, it logged an error message
    (void) createDepositInvoice(date, (int)zoneId, (int)registrar, (long)price); 
  };
...
#define INVOICE_XML_FILE_TYPE 2 
  ...
  public:
    ...
    virtual void reload() {
        ...
        // TODO auto_ptr just to avoid changing -> into . in 1000 places :)
