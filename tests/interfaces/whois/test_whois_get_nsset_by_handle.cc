...
bool is_member(const std::string &_fqdn, const std::vector< Fred::DnsHost > &_hosts)
{
...
const Fred::DnsHost& get_member(const std::string &_fqdn, const std::vector< Fred::DnsHost > &_hosts)
{
    for (::size_t idx = 0; idx < _hosts.size(); ++idx)
    {
        if (_hosts[idx].get_fqdn() == _fqdn)
...
BOOST_FIXTURE_TEST_CASE(get_nsset_by_handle, get_nsset_by_handle_fixture)
{
    ...
    for (::size_t nserver_idx = 0; nserver_idx < nss.nservers.size(); ++nserver_idx)
    {
        ...
        if (has_dns_host)
        {
            const Fred::DnsHost &dns_host = get_member(nss.nservers[nserver_idx].fqdn, nsset.dns_hosts);
