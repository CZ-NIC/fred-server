...
: whois_impl_instance_fixture
{
      ...
      test_limit(10)
    {
        for(unsigned int i = 0; i < test_limit; ++i)
        {
            const Fred::InfoNssetData& ind = Test::exec(
                    Test::CreateX_factory<Fred::CreateNsset>().make(registrar.handle)
                        .set_dns_hosts(Util::vector_of<Fred::DnsHost>(Fred::DnsHost(test_fqdn, Util::vector_of<boost::asio::ip::address>(boost::asio::ip::address()))))
        ...
        for(unsigned int i = 0; i < 3; ++i)
        {
            Test::exec(
                    Test::CreateX_factory<Fred::CreateNsset>().make(registrar.handle)
                        .set_dns_hosts(Util::vector_of<Fred::DnsHost>(Fred::DnsHost("other-fqdn", Util::vector_of<boost::asio::ip::address>(boost::asio::ip::address()))))
                        .set_tech_contacts(Util::vector_of<std::string>(contact.handle)),
                    ctx);
        }
...
BOOST_FIXTURE_TEST_CASE(get_nssets_by_ns, get_nssets_by_ns_fixture)
{
        ...
        ++it)
    {
        ...
        BOOST_CHECK(it->tech_contact_handles.at(0) == found->second.tech_contacts.at(0).handle);
    }
}
