      ...
      test_limit(10)
    {
        ...
        for(unsigned long i = 0; i < 3; ++i)
        {
            Test::exec(
                    Test::CreateX_factory<Fred::CreateKeyset>().make(registrar.handle)
                        .set_dns_keys(Util::vector_of<Fred::DnsKey>(Fred::DnsKey(42, 777, 13, "any-key")))//what key has to be here?
                        .set_tech_contacts(Util::vector_of<std::string>(Test::contact::make(ctx).handle)),//random contact
                    ctx);
        }
...
BOOST_FIXTURE_TEST_CASE(get_keysets_by_tech_c, get_keysets_by_tech_c_fixture)
{
    ...
    for(std::vector<Registry::WhoisImpl::KeySet>::iterator it = ks_s.content.begin(); it != ks_s.content.end(); ++it)
    {
        ...
        BOOST_CHECK(it->dns_keys.at(0).flags      == found.dns_keys.at(0).get_flags());
        BOOST_CHECK(it->dns_keys.at(0).protocol   == found.dns_keys.at(0).get_protocol());
        BOOST_CHECK(it->dns_keys.at(0).public_key == found.dns_keys.at(0).get_key());
        ...
        BOOST_CHECK(it->tech_contact_handles.at(0) == found.tech_contacts.at(0).handle);
    }
}
...
BOOST_FIXTURE_TEST_CASE(get_keysets_by_tech_c_no_contact, get_keysets_by_tech_c_fixture)
{
    try
    {
        Registry::WhoisImpl::KeySetSeq ks_s = impl.get_keysets_by_tech_c("fine-tech-c-handle", test_limit);
...
BOOST_FIXTURE_TEST_CASE(get_keysets_by_tech_c_wrong_contact, get_keysets_by_tech_c_fixture)
{
    try
    {
        Registry::WhoisImpl::KeySetSeq ks_s = impl.get_keysets_by_tech_c("", test_limit);
