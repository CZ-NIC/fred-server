...
BOOST_FIXTURE_TEST_CASE(block_unblock_transfer_and_update_email, lock_request_fixture)
{
    ...
    BOOST_CHECK_EQUAL(
            get_db_public_request(Fred::OperationContextCreator(), unblock_change, 8, 0).size(),
            1);
}
...
BOOST_FIXTURE_TEST_CASE(block_unblock_transfer_and_update_post, lock_request_fixture) // check for fail blocks
{
    Fred::OperationContextCreator ctx;
    Registry::PublicRequestImpl::PublicRequest pr;
    unsigned long long block_change = pr.create_block_unblock_request(
            ...
            Registry::PublicRequestImpl::BLOCK_TRANSFER_AND_UPDATE);
    BOOST_REQUIRE_EQUAL(
            get_db_public_request(ctx, block_change, 5, 0).size(),
            1);
    ...
    unsigned long long unblock_change = pr.create_block_unblock_request(
            ...
            Registry::PublicRequestImpl::UNBLOCK_TRANSFER_AND_UPDATE);
    BOOST_CHECK_EQUAL(
            get_db_public_request(Fred::OperationContextCreator(), unblock_change, 9, 0).size(),
            1);
}
