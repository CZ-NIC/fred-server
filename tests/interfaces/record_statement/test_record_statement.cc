...
BOOST_FIXTURE_TEST_CASE(domain_printout_xml, Test::domain_fixture)
{
    try
    {
        ...
        const Fred::InfoRegistrarOutput info_sponsoring_registrar_output = Fred::InfoRegistrarByHandle(
                info_domain_output.info_domain_data.sponsoring_registrar_handle).exec(ctx, "UTC");

        for (std::vector<Fred::ObjectIdHandlePair>::const_iterator itr = info_domain_output.info_domain_data.admin_contacts.begin();
             itr != info_domain_output.info_domain_data.admin_contacts.end(); ++itr)
        {
            info_admin_contact_output.push_back(
                Fred::InfoContactByHandle(itr->handle).exec(ctx, "UTC"));
        }
        ...
        const boost::optional<Fred::RecordStatement::Impl::NssetPrintoutInputData> nsset_data =
                Fred::RecordStatement::Impl::make_nsset_data(nsset_handle, ctx);
        ...
        const boost::optional<Fred::RecordStatement::Impl::KeysetPrintoutInputData> keyset_data =
                Fred::RecordStatement::Impl::make_keyset_data(keyset_handle, ctx);
        ...
        const std::string test_domain_xml = Fred::RecordStatement::Impl::domain_printout_xml<Tz::Europe::Prague>(
                ctx,
                info_domain_output,
                data_valid_at,
                ...
                nsset_data,
                keyset_data,
                Fred::RecordStatement::Impl::make_external_states(info_domain_output.info_domain_data.id, ctx));
...
BOOST_FIXTURE_TEST_CASE(nsset_printout_xml, Test::domain_fixture)
{
    try
    {
        ...
        boost::optional<Fred::RecordStatement::Impl::NssetPrintoutInputData> nsset_data
            = Fred::RecordStatement::Impl::make_nsset_data(nsset_handle, ctx);
        ...
        const std::string test_nsset_xml = Fred::RecordStatement::Impl::nsset_printout_xml(
                *nsset_data,
                data_valid_at);
...
BOOST_FIXTURE_TEST_CASE(keyset_printout_xml, Test::domain_fixture)
{
    try
    {
        ...
        boost::optional<Fred::RecordStatement::Impl::KeysetPrintoutInputData> keyset_data
            = Fred::RecordStatement::Impl::make_keyset_data(keyset_handle, ctx);
        ...
        const std::string test_keyset_xml = Fred::RecordStatement::Impl::keyset_printout_xml(
                *keyset_data,
                data_valid_at);
...
BOOST_FIXTURE_TEST_CASE(contact_printout_xml, Test::domain_fixture)
{
    try
    {
        ...
        const std::string test_contact_xml = Fred::RecordStatement::Impl::contact_printout_xml<Tz::Europe::Prague>(
              ctx,
              info_contact_output,
              data_valid_at,
              Registry::RecordStatement::Purpose::private_printout,
              info_sponsoring_registrar_output,
              Fred::RecordStatement::Impl::make_external_states(info_contact_output.info_contact_data.id, ctx));
