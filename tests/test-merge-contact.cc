...
/**
 * test MergeContact with blocked nsset
 */
BOOST_FIXTURE_TEST_CASE(merge_contact_with_blocked_nsset, merge_contact_domain_fixture)
{
    unsigned long long nsset_id = Fred::InfoNssetByHandle(
            test_nsset_handle).exec(ctx).info_nsset_data.id;
    if(!Fred::ObjectHasState(nsset_id,
        Fred::ObjectState::SERVER_BLOCKED).exec(ctx))
    {
        ctx.get_conn().exec_params(
            "INSERT INTO object_state_request (object_id, state_id)"
            " VALUES ($1::integer, (SELECT id FROM enum_object_states"
        ...
        Fred::PerformObjectStateRequest().set_object_id(nsset_id).exec(ctx);
        ctx.commit_transaction();
    }
    ...
    {
        BOOST_CHECK(ex.is_set_object_blocked());
        BOOST_CHECK(ex.get_object_blocked().compare(test_nsset_handle) == 0);
    }
...
/**
 * test MergeContact with blocked keyset
 */
BOOST_FIXTURE_TEST_CASE(merge_contact_with_blocked_keyset, merge_contact_domain_fixture)
{
    unsigned long long keyset_id = Fred::InfoKeysetByHandle(
            test_keyset_handle).exec(ctx).info_keyset_data.id;
    if(!Fred::ObjectHasState(keyset_id,
        Fred::ObjectState::SERVER_BLOCKED).exec(ctx))
    {
        ctx.get_conn().exec_params(
            "INSERT INTO object_state_request (object_id, state_id)"
            ...
            Database::query_param_list
                (keyset_id)
                (Fred::ObjectState::SERVER_BLOCKED));
        Fred::PerformObjectStateRequest().set_object_id(keyset_id).exec(ctx);
        ctx.commit_transaction();
    }
    ...
    {
        BOOST_CHECK(ex.is_set_object_blocked());
        BOOST_CHECK(ex.get_object_blocked().compare(test_keyset_handle) == 0);
    }
