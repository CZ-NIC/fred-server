...
class UpdateContact
{
    ...
    typedef typename Fred::ExceptionTraits<DERIVED>::Exception ExceptionType;
    ExceptionType* ex_ptr_;
...
public:
    ...
    DERIVED& set_exception(ExceptionType* ex_ptr)
    {
        ex_ptr_ = ex_ptr;
        return static_cast<DERIVED&>(*this);
    }

    void exec(Fred::OperationContext& ctx, const Fred::InfoContactOutput& contact)
    {
        //update contact
        //...
        //error handling
        if(ex_ptr_ != 0 &&/*unknown ssntype*/true)
            ex_ptr_->set_unknown_ssntype("some_unknown_ssntype_to_be_reported")
            ;
...
template <>
struct Fred::ExceptionTraits<UpdateContactById>
{
...
class UpdateContactById
:   public Fred::ExceptionTraits<UpdateContactById>,
...
public:
    ...
    , id_(id)
    , select_contact_(Fred::InfoContact().set_id(id_).set_history_query(false))
    {}

    void exec(Fred::OperationContext& ctx)
    {
        ExceptionTraits<UpdateContactById>::Exception ex;

        std::vector<Fred::InfoContactOutput> contact = select_contact_.exec(ctx);
        if(contact.empty()) ex.set_unknown_contact_id(id_);
        UpdateContact<UpdateContactById>::set_exception(&ex);
        UpdateContact<UpdateContactById>::exec(ctx,contact.at(0));
        if(ex.throw_me())
        {
            BOOST_MESSAGE("throw exception:");
