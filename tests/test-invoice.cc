...
void create2Invoices(Fred::Invoicing::Manager *man, Database::Date taxdate, Database::ID zone_id, Database::ID reg_id, Money amount)
{
    Money out_credit;
        ...
        Database::ID zone_id)
{
    ...
    Database::ID invoiceid = invMan->createDepositInvoice(
                    ...
                    , boost::posix_time::ptime(taxdate)
                    , out_credit);//price
    Fred::Credit::add_credit_to_invoice( reg->getId(),  zone_id, out_credit, invoiceid);
...
BOOST_AUTO_TEST_CASE( chargeDomain )
{
    ...
    Database::ID invoiceid = invMan->createDepositInvoice(taxdate //taxdate
                    ...
                    , boost::posix_time::ptime(taxdate)
                    , out_credit);//price
    Fred::Credit::add_credit_to_invoice( registrar->getId(),  zone_cz_id, out_credit, invoiceid);
    BOOST_CHECK_EQUAL(invoiceid != 0,true);
    ...
    Database::ID invoiceid2 = invMan->createDepositInvoice(taxdate //taxdate
                    ...
                    , boost::posix_time::ptime(taxdate)
                    , out_credit);//price
    Fred::Credit::add_credit_to_invoice( registrar->getId(),  zone_enum_id, out_credit, invoiceid2);
    BOOST_CHECK_EQUAL(invoiceid2 != 0,true);
